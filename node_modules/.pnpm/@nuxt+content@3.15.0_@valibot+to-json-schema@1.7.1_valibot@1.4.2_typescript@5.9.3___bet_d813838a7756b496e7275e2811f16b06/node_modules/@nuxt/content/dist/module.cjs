...
const ctx = unctx.getContext("@nuxt/content:validators-context");
if (!ctx.tryUse()) {
  ctx.set({
    zod3: {
...
async function isPackageInstalled(packageName) {
  try {
    await pkgTypes.resolvePackageJSON(packageName, { from: (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('module.cjs', document.baseURI).href)) });
    return true;
  } catch {
    return false;
  }
...
async function initiateValidatorsContext() {
  if (await isPackageInstalled("valibot") && await isPackageInstalled("@valibot/to-json-schema")) {
    nuxtContentContext().set("valibot", await import('./chunks/valibot.cjs'));
  }
  if (await isPackageInstalled("zod")) {
    nuxtContentContext().set("zod3", await Promise.resolve().then(function () { return zod3; }));
    nuxtContentContext().set("zod4", await import('./chunks/zod4.cjs'));
  }
}
    ...
    return [
      ...
      ...await Promise.all(
        publicCollections.map(async (c) => {
          const type = await jsonSchemaToTypescriptLite.compile(c.schema, "CLASS").then((code) => code.replace("export interface CLASS", `interface ${scule.pascalCase(c.name)}CollectionItem extends ${parentInterface(c)}`));
...
function watchContents(nuxt, options, manifest) {
  ...
  async function onChange(pathOrError) {
    ...
    const absolutePath = pathe.resolve(pathOrError);
    const matches = sourceMap.filter(({ source, cwd }) => {
      if (cwd && absolutePath.startsWith(cwd)) {
        return micromatch__default.isMatch(absolutePath.substring(cwd.length), source.include, { ignore: source.exclude || [], dot: true });
      }
      return false;
    });
    if (matches.length) {
      ...
      for (const match of matches) {
        ...
        if (content === void 0) {
          content = await promises.readFile(fullPath, "utf8");
          if (content === "") {
...
async function configureMDCModule(contentOptions, nuxt) {
  ...
  setMdcConfigResolver(async () => {
    if (!_configs) {
      ...
      const jiti$1 = jiti.createJiti(nuxt.options.rootDir);
      _configs = paths.length ? await Promise.all(paths.map((path) => jiti$1.import(path).then((m) => m.default || m))) : [];
    }
    return _configs;
  });
...
const nuxthub = definePreset({
  ...
  async setup(options, nuxt, config) {
    ...
    if (preset.includes("vercel")) {
      options._remoteSourceRootDir = options._remoteSourceRootDir || await promises.mkdtemp(pathe.join(node_os.tmpdir(), "nuxt-content"));
    }
  },
