...
import { tryUseNuxt, resolveAlias, useLogger, updateTemplates, isIgnored, useNuxt, resolveModule, hasNuxtModule, extendViteConfig, addTemplate, addServerHandler, addPlugin, installModule, addComponent, addPrerenderRoutes, defineNuxtModule, createResolver, addImports, addServerImports, addTypeTemplate, addVitePlugin } from '@nuxt/kit';
...
import { getOrderedSchemaKeys, describeProperty, getCollectionFieldsTypes } from '../dist/runtime/internal/schema.js';
import fs, { createReadStream } from 'node:fs';
import { withLeadingSlash, withoutTrailingSlash, isRelative, withTrailingSlash } from 'ufo';
import { glob } from 'tinyglobby';
import { readGitConfig, resolvePackageJSON } from 'pkg-types';
...
const ctx = getContext("@nuxt/content:validators-context");
if (!ctx.tryUse()) {
  ctx.set({
    zod3: {
...
const nuxtContentContext = ctx.use;
...
async function initiateValidatorsContext() {
  if (await isPackageInstalled("valibot") && await isPackageInstalled("@valibot/to-json-schema")) {
    nuxtContentContext().set("valibot", await import('./chunks/valibot.mjs'));
  }
  if (await isPackageInstalled("zod")) {
    nuxtContentContext().set("zod3", await Promise.resolve().then(function () { return zod3; }));
    nuxtContentContext().set("zod4", await import('./chunks/zod4.mjs'));
  }
}
...
let _getMdcConfigs;
function setMdcConfigResolver(fn) {
  _getMdcConfigs = fn;
}
let highlightPlugin;
...
async function createParser(collection, nuxt) {
  ...
  return async function parse(file) {
    ...
    const beforeParseCtx = { file, collection, parserOptions };
    ...
    const pathMetaFields = await pathMetaTransformer.transform(parsedContent, beforeParseCtx.parserOptions?.pathMeta);
  ...
  return {
    ...
    getContents: ({ app, nuxt, options }) => {
      const componentsMap = app.components.filter((c) => {
        ...
        const importPath = isAbsolute(c.filePath) ? "./" + relative(join(nuxt.options.buildDir, "content"), c.filePath).replace(/\b\.(?!vue)\w+$/g, "") : c.filePath.replace(/\b\.(?!vue)\w+$/g, "");
...
const NuxtContentHMRUnplugin = createUnplugin((opts) => {
...
function watchContents(nuxt, options, manifest) {
  ...
  async function onChange(pathOrError) {
    ...
    const absolutePath = resolve(pathOrError);
    ...
    if (matches.length) {
      ...
      for (const match of matches) {
        ...
        if (content === void 0) {
          ...
          if (content === "") {
            content = await new Promise((resolve2) => setTimeout(resolve2, 50)).then(() => readFile(fullPath, "utf8"));
          }
        }
  ...
  const resolvedSource = {
    ...
    getKeys: async () => {
      ...
      return new Promise((resolve) => {
        ...
        createReadStream(join(resolvedSource.cwd, fixed, keys[0])).on("data", function(chunk) {
...
function replaceComponentSchemas(property2) {
  ...
  if ($content?.inherit) {
    ...
    const schema = propsToJsonSchema(meta.props);
    return {
      ...schema,
...
async function loadContentConfig(nuxt, options) {
  ...
  const loader = watch ? (opts) => watchConfig({
    ...opts,
    onWatch: (e) => {
...
async function configureMDCModule(contentOptions, nuxt) {
  ...
  setMdcConfigResolver(async () => {
    if (!_configs) {
      ...
      const jiti = createJiti(nuxt.options.rootDir);
      _configs = paths.length ? await Promise.all(paths.map((path) => jiti.import(path).then((m) => m.default || m))) : [];
    }
    return _configs;
  });
...
const vercel = definePreset({
  ...
  async setup(options, nuxt) {
    options.database ||= { type: "sqlite", filename: "/tmp/contents.sqlite" };
    const vercelRuntime = nuxt.options.nitro?.vercel;
...
async function setupPreview(_options, nuxt, resolver, manifest) {
  nuxt.hook("schema:resolved", (schema) => {
    const template = addTemplate(previewTemplate(manifest.collections, {}, schema)).dst;
...
function toJSONSchema(_schema, name) {
  ...
  const jsonSchema = zodToJsonSchema(schema, { name, $refStrategy: "none", dateStrategy: "format:date" });
...
const module$1 = defineNuxtModule({
  ...
  moduleDependencies(nuxt) {
    const nuxtOptions = nuxt.options;
    const contentOptions = defu$1(nuxtOptions.content, moduleDefaults);
    return {
      "@nuxtjs/mdc": {
        overrides: {
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.vite.optimizeDeps = defu$1(nuxt.options.vite.optimizeDeps, {
    ...
    nuxt.options.alias = defu$1(nuxt.options.alias, {
    ...
    nuxt.hook("nitro:config", async (config) => {
      ...
      const sqliteConnector = options.experimental?.sqliteConnector || (options.experimental?.nativeSqlite ? "native" : void 0);
      ...
      const localSqliteConnector = sqliteConnector === "bun" && !process.versions.bun ? void 0 : sqliteConnector;
...
async function processCollectionItems(nuxt, collections, options) {
  ...
  const requestedConnector = options.experimental?.sqliteConnector || (options.experimental?.nativeSqlite ? "native" : void 0);
  const buildConnector = requestedConnector === "bun" && !process.versions.bun ? void 0 : requestedConnector;
  ...
  for await (const collection of collections) {
    ...
    for await (const source of collection.source) {
      if (source.prepare) {
        const rootDir = (source.repository ? options._remoteSourceRootDir : collection.__rootDir) || nuxt.options.rootDir;
  ...
  ].map((tag) => getMappedTag(tag, options?.renderer?.alias)).filter((tag) => !htmlTags.has(kebabCase(tag))).map((tag) => pascalCase(tag));
...
export { module$1 as default, defineCollection, defineCollectionSource, defineContentConfig, defineTransformer, metaStandardSchema, pageStandardSchema, property, z };
